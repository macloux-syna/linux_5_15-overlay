...
struct drm_connector *syna_dvi_connector_create(struct drm_device *dev)
{
	...
	drm_connector_init(dev, connector, &syna_dvi_connector_funcs,
	...
	for (i = 0; i < ARRAY_SIZE(syna_extra_modes); i++) {
		syna_vpp_set_res_limit(dev, syna_extra_modes[i].hdisplay,
			syna_extra_modes[i].vdisplay);
	}
