...
static int vpp_is_recovery;
static int vpp_is_vpp_ta;
...
void vpp_force_enable_recovery(bool enable)
{
	...
	if (enable)
		vpp_is_recovery = enable;
}
EXPORT_SYMBOL(vpp_force_enable_recovery);

int VPP_Is_Vpp_Ta(void)
{
	return vpp_is_vpp_ta;
}
EXPORT_SYMBOL(VPP_Is_Vpp_Ta);

int VPP_Is_Recovery_Mode(void)
{
	return vpp_is_recovery;
}
...
static void vpp_read_config(struct platform_device *pdev)
{
	...
	pr_info("%s:%d: enable_ampless_boot - %x, is_recovery - %x\n",
		__func__, __LINE__, enable_ampless_boot, vpp_is_recovery);
}
